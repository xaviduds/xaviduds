<table>
  <tr>
    <td colspan="3" align="center">
      <strong>Class</strong>
    </td>
  </tr>
  <tr>
    <td><strong>Primary</strong></td>
    <td colspan="2">Frontline Spellweaver (Novice)</td>
  </tr>
  <tr>
    <td><strong>Secondary</strong></td>
    <td colspan="2">Spell Owner (Adept)</td>
  </tr>
  <tr>
    <td colspan="3" align="center">
      <strong>Profession</strong>
    </td>
  </tr>
  <tr>
    <td><strong>Student</strong></td>
    <td colspan="2">Pondering orbs at the wizard tower of <a href="https://roadmap.sh/u/xaviduds">Roadmapland</a>.</td>
  </tr>
 <tr>
    <td colspan="3" align="center">
      <strong>Familiar</strong>
    </td>
  </tr>
  <tr>
    <td>
      <a href="https://github.com/lince-social/lince">Lince</a>
    </td>
    <td colspan="2" >
      <img src="https://github.com/lince-social/.github/blob/main/media/logo/branco_no_preto.png" width="95px" align="left">A robotic companion for everything. It models your Needs and what you can do for others (Contributions), binding a quantity to every phenomena, allowing for manipulation and logic building. Facilitating all economic trades, organizing human interactions, automating computable tasks. It is still a cub, but becomes powerful if nurtured with commits and blue elephants.</td>
  </tr>
 
  <tr>
    <td colspan="3" align="center">
      <strong>Arsenal</strong><br>
    </td>
  </tr>
  <tr>
    <td><strong>Ice Staff</strong></td>
    <td colspan="2"><img src="nixos.gif" width="20px" align="left">Made from flakes of Nix ice, infused with a crystal containing Tux, the Penguin God's soul.</td>
  </tr>
 <tr>
    <td rowspan="12">
      <strong>Grimoire</strong>
    </td>
  </tr>
  <!--
   <tr>
    <td>Automaton & Divination</td>
    <td>scikit-learn, TensorFlow, Pytorch</td>
  </tr>
  -->
   <tr>
    <td><em>Weightcrafting</em></td>
    <td>scikit-learn</td>
  </tr>
  <tr>
    <td>Alquery</td>
    <td>PostgreSQL</td>
  </tr>
  <tr>
    <td><em>Rune Enchantment</em></td>
    <td>Python (Pandas, Plotly, Flask), JavaScript/TypeScript (React, Next.js, Node.js) [Channeling...]</td>
  </tr>
  <tr>
    <td><em>Conjuration</em></td>
    <td>HTML</td>
  </tr>
  <tr>
    <td><em>Illusion</em></td>
    <td>CSS, Tailwind CSS</td>
  </tr>
  <tr>
    <td><em>Retromancy</em></td>
    <td>Git, GitHub</td>
  </tr>
  <tr>
    <td><em>Nimbusforging</em></td>
    <td>Vercel, GitHub Pages</td>
  </tr>
    <tr>
    <td><em>Chaos Weaving</em></td>
    <td>Obsidian, Notion, Azure DevOps Boards, ClickUp, Jira</td>
  </tr>
  <tr>
    <td>Incantation</td>
    <td>Helix, Neovim, Emacs, VS Code</td>
  </tr>
  <tr>
    <td><em>Clairvoyance</em></td>
    <td>Qlik</td>
  </tr>
    <tr>
    <td><em>Mundanology</em></td>
    <td>Office Apps: the Green, the Blue, the Orange. Canva, GIMP, Photopea, Audacity</td>
  </tr>
    <tr>
    <td colspan="3" align="center">
      <strong>Spells</strong>
    </td>
  </tr>
  <tr>
    <td><strong>CRUD</strong></td>
    <td colspan="2">The simplest enterprise spell, the first anyone learns. The sorcerer conjures a blue elephant to remember, speak and take actions.</td>
  </tr>
  <tr>
    <td><strong>Machine Learning</strong></td>
    <td colspan="2">k-NN, Linear/Logistic Regression, Decision Trees, Neural Networks (Novice)</td>
  </tr>
    <tr>
    <td><strong>Linux From Scratch</strong></td>
    <td colspan="2">The art of creating magical wands and staffs from scratch. The sourcerer needs to have a deep understanding of runes and magic circles. Currently immersed in scrolls to master it and craft my own.</td>
  </tr>
    <tr>
    <td colspan="3" align="center">
      <strong>Skills</strong>
    </td>
  </tr>
  <tr>
    <td><strong>Gardening</strong></td>
    <td colspan="2">Passive: Ability of pulling life out of the dirt, or burying it, increases by 40%.</td>
  </tr>
   <tr>
    <td><strong>Meditation</strong></td>
    <td colspan="2">Active - Jhāna [Channel Time: random, Success Chance: random, Duration: random]: Consciousness is your only sense, +30MMP (mental mana points). Passive: Desire of pizza decreases by 30%.</td>
  </tr>
    <tr>
    <td><strong>Minimalism (Corrupted)</strong></td>
    <td colspan="2">Passive: Increase cleaning speed by 300%. Corruption: aversion to unuseful items.</td>
  </tr>
  <!--
    <tr>
    <td colspan="3" align="center">
      <strong>Background</strong><br>
    </td>
  </tr>
  <tr>
    <td colspan="3">Ice Staff</td>
  </tr>
  -->
</table>

<table>
  <tr>
    <td colspan="3" align="center">
      <details>
        <summary><strong>Class</strong></summary>
        <table>
          <tr>
            <td><strong>Primary</strong></td>
            <td colspan="2">Frontline Spellweaver (Novice)</td>
          </tr>
          <tr>
            <td><strong>Secondary</strong></td>
            <td colspan="2">Spell Owner (Adept)</td>
          </tr>
        </table>
      </details>
    </td>
  </tr>
  <tr>
    <td colspan="3" align="center">
      <details>
        <summary><strong>Profession</strong></summary>
        <table>
          <tr>
            <td><strong>Student</strong></td>
            <td colspan="2">Pondering orbs at the wizard tower of <a href="https://roadmap.sh/u/xaviduds">Roadmapland</a>.</td>
          </tr>
        </table>
      </details>
    </td>
  </tr>
  <tr>
    <td colspan="3" align="center">
      <details>
        <summary><strong>Familiar</strong></summary>
        <table>
          <tr>
            <td><a href="https://github.com/lince-social/lince">Lince</a></td>
            <td colspan="2">
              <img src="https://github.com/lince-social/.github/blob/main/media/logo/branco_no_preto.png" width="95px" align="left">
              A robotic companion for everything. It models your Needs and what you can do for others (Contributions), binding a quantity to every phenomenon, allowing for manipulation and logic building. Facilitating all economic trades, organizing human interactions, automating computable tasks. It is still a cub, but becomes powerful if nurtured with commits and blue elephants.
            </td>
          </tr>
        </table>
      </details>
    </td>
  </tr>
  <tr>
    <td colspan="3" align="center">
      <details>
        <summary><strong>Arsenal</strong></summary>
        <table>
          <tr>
            <td><strong>Ice Staff</strong></td>
            <td colspan="2">
              <img src="nixos.gif" width="20px" align="left">
              Made from flakes of Nix ice, infused with a crystal containing Tux, the Penguin God's soul.
            </td>
          </tr>
        </table>
      </details>
    </td>
  </tr>
  <tr>
    <td colspan="3" align="center">
      <details>
        <summary><strong>Grimoire</strong></summary>
        <table>
          <tr>
            <td rowspan="12"><strong>Grimoire</strong></td>
          </tr>
          <tr>
            <td><em>Weightcrafting</em></td>
            <td>scikit-learn</td>
          </tr>
          <tr>
            <td>Alquery</td>
            <td>PostgreSQL</td>
          </tr>
          <tr>
            <td><em>Rune Enchantment</em></td>
            <td>Python (Pandas, Plotly, Flask), JavaScript/TypeScript (React, Next.js, Node.js) [Channeling...]</td>
          </tr>
          <tr>
            <td><em>Conjuration</em></td>
            <td>HTML</td>
          </tr>
          <tr>
            <td><em>Illusion</em></td>
            <td>CSS, Tailwind CSS</td>
          </tr>
          <tr>
            <td><em>Retromancy</em></td>
            <td>Git, GitHub</td>
          </tr>
          <tr>
            <td><em>Nimbusforging</em></td>
            <td>Vercel, GitHub Pages</td>
          </tr>
          <tr>
            <td><em>Chaos Weaving</em></td>
            <td>Obsidian, Notion, Azure DevOps Boards, ClickUp, Jira</td>
          </tr>
          <tr>
            <td>Incantation</td>
            <td>Helix, Neovim, Emacs, VS Code</td>
          </tr>
          <tr>
            <td><em>Clairvoyance</em></td>
            <td>Qlik</td>
          </tr>
          <tr>
            <td><em>Mundanology</em></td>
            <td>Office Apps: the Green, the Blue, the Orange. Canva, GIMP, Photopea, Audacity</td>
          </tr>
        </table>
      </details>
    </td>
  </tr>
  <tr>
    <td colspan="3" align="center">
      <details>
        <summary><strong>Spells</strong></summary>
        <table>
          <tr>
            <td><strong>CRUD</strong></td>
            <td colspan="2">The simplest enterprise spell, the first anyone learns. The sorcerer conjures a blue elephant to remember, speak and take actions.</td>
          </tr>
          <tr>
            <td><strong>Machine Learning</strong></td>
            <td colspan="2">k-NN, Linear/Logistic Regression, Decision Trees, Neural Networks (Novice)</td>
          </tr>
          <tr>
            <td><strong>Linux From Scratch</strong></td>
            <td colspan="2">The art of creating magical wands and staffs from scratch. The sourcerer needs to have a deep understanding of runes and magic circles. Currently immersed in scrolls to master it and craft my own.</td>
          </tr>
        </table>
      </details>
    </td>
  </tr>
  <tr>
    <td colspan="3" align="center">
      <details>
        <summary><strong>Skills</strong></summary>
        <table>
          <tr>
            <td><strong>Gardening</strong></td>
            <td colspan="2">Passive: Ability of pulling life out of the dirt, or burying it, increases by 40%.</td>
          </tr>
          <tr>
            <td><strong>Meditation</strong></td>
            <td colspan="2">Active - Jhāna [Channel Time: random, Success Chance: random, Duration: random]: Consciousness is your only sense, +30MMP (mental mana points). Passive: Desire of pizza decreases by 30%.</td>
          </tr>
          <tr>
            <td><strong>Minimalism (Corrupted)</strong></td>
            <td colspan="2">Passive: Increase cleaning speed by 300%. Corruption: aversion to unuseful items.</td>
          </tr>
        </table>
      </details>
    </td>
  </tr>
</table>
